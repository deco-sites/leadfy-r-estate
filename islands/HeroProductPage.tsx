  ...
  return (
    <div className="container mt-28 mx-auto px-4 py-16 lg:px-0">
      <div id={id} className="flex flex-col md:flex-row justify-between space-x-4 lg:px-[5%]">
        ...
        <GalleryProductPage images={property[0]?.images} />
...
function GalleryProductPage({ images }: { images: string[] }) {
  ...
  return (
    <div id={id} className="flex flex-col md:flex-row items-start gap-6 w-full">
      <div className="w-full md:w-2/3 max-w-[1000px] relative">
      ...
      {/* Formulário ao lado (aparece em telas médias e grandes) */}
      <div className="hidden md:block w-1/3">
        <FormProductPage />
      </div>
    </div>
...
function Buttons() {
  return (
    <>
      <div className="flex items-center justify-center z-10 col-start-1 row-start-2 absolute left-0 top-[15%]">
        <Slider.PrevButton>
          <Icon
      ...
      <div className="flex items-center justify-center z-10 col-start-3 row-start-2 absolute right-0 top-[15%]">
        <Slider.NextButton>
          <Icon
            className="text-white"
            size={26}
